...
class Log:
    def __init__(self):
        # proDir = os.getcwd()     #os.getcwd()表示当前路径，"../.."表示上上级路径，这个和执行它的文件的位置有关
        # proDir = os.path.abspath(os.path.join(os.getcwd(), "..")) # 上级路径
        proDir = readConfig.proDir                  #这个是固定保存位置
    ...
    '''这里用到了可以使用@property装饰器，可以把方法变成属性，原本方法调用需要get_logger(),现在只要get_logger就可以'''
    def get_logger(self):
        """获取记录
        get logger
        :return:
